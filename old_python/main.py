...
def get_toshl_cats_and_tags():
  r = requests.get(
    "https://api.toshl.com/categories?per_page=500", headers=toshl_headers)
  ...
  r = requests.get("https://api.toshl.com/tags?per_page=500",
           headers=toshl_headers)
...
def get_similar_toshl_entries(splitwise_entry, toshl_entries):
  ...
  for te in toshl_entries:
    ...
    similar_price = abs(
      abs(abs(te['amount']) - abs(float(e['share_amount'])) / abs(te['amount']))) < 0.2
...
def friend_transfer_page():
  clear()
  print(f"Friend Transfer (Friends with balances)")
  print(f"==========================================")
  print("")
  print("Loading friends list:")
  r = requests.get(
    'https://secure.splitwise.com/api/v3.0/get_friends', headers=splitwise_headers)
  ...
  for f in friends_arr:
    if (len(f['balance']) > 0):
      full_name = ', '.join(
        filter(None, (f['first_name'], f['last_name'])))
      print(
        f"[{str(ind)}] {full_name} {f['balance'][0]['amount']} {f['balance'][0]['currency_code']}")
      friends_with_balances.append(f)
      ind = ind + 1

  print("Enter '>' to load settled up friends")
  selected_friend = None
  while (True):
    i = input()
    if (i == '>'):
      break
    elif (int(i) < 0 or int(i) >= len(friends_with_balances)):
      print('Invalid input!')
  ...
  if (selected_friend is None):
    settled_friend_transfer_page()
  else:
    page = 1
    friend_expenses_page_list(selected_friend, page)


def settled_friend_transfer_page():
  clear()
  print(f"Friend Transfer (settled up friends)")
  print(f"==========================================")
  print("")
  print("Loading friends list:")
  r = requests.get(
  ...
  for f in friends_arr:
    if (len(f['balance']) <= 0):
      ...
      print(
        f"[{str(ind)}] {full_name}")
      friends_without_balances.append(f)
      ind = ind + 1

  print("Enter '<' to load unsettled up friends")
  selected_friend = None
  while (True):
    i = input()
    if (i == '<'):
      break
    elif (int(i) < 0 or int(i) >= len(friends_without_balances)):
      print('Invalid input!')
      input("Press Enter to try again")
    else:
      selected_friend = friends_without_balances[int(i)]
      break
...
def friend_expenses_page_list(selected_friend, page):
  while (True):  # User input
    clear()
    full_name = ' '.join(
      filter(None, (selected_friend['first_name'], selected_friend['last_name'])))
...
def get_category(breadcrumb, expense):
  ...
  while (True):  # Paginate category list
    ...
    print("")
    print(
      f"Choose your category (showing {offset+1} to {offset+page_size+1})")
...
def get_tag(breadcrumb, expense, selected_category):
  ...
  while (True):  # Paginate tags list
    ...
    print(
      f"Choose your tag (showing {offset+1} to {offset+page_size+1})\n(You can only choose one, sorry. To add more use the app.s)")
    ind = 0
...
print("done")
splitwise_headers = {"content-type": "application/json",
           "Authorization": "Bearer " + auth_dict['splitwise_api_key']}
toshl_headers = {"content-type": "application/json",
         "Authorization": "Bearer " + auth_dict['toshl_api_key']}


def get_friend_expenses(friend_id, count, page):
  r = requests.get(
    f'https://secure.splitwise.com/api/v3.0/get_expenses?friend_id={friend_id}&limit={count}&offset={page * count}', headers=splitwise_headers)
...
r = requests.get(
  'https://secure.splitwise.com/api/v3.0/get_current_user', headers=splitwise_headers)
...
print(
  f'  Splitwise account: {user_accounts["splitwise"]["email"]} ({user_accounts["splitwise"]["id"]})')
print(
  f'  Toshl account: {user_accounts["toshl"]["email"]} ({user_accounts["toshl"]["id"]})')
print("\n")
input("Press Enter to continue...")
